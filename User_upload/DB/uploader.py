...
def _to_row(chunk: dict, inspection_no: int) -> dict:
...
def to_export_rows(chunks: list[dict], inspection_no: int) -> list[dict]:
    return [_to_row(c, inspection_no) for c in chunks]
...
def upload_chunks(chunks: list[dict], inspection_no: int | None = None) -> int:
    client = _get_client()
    if inspection_no is None:
        inspection_no = _next_inspection_no()
    rows = [_to_row(c, inspection_no) for c in chunks]
